...
For an interval $[lo, hi)$, we can binary search for the first value that will have $f$ return $\text{false}$ and then subtract $1$ to get $\text{last\_true}$.

This can be easily modified to $text{first\_true}$ by inverting the return value of $f$.
