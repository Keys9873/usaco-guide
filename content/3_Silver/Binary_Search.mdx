...
For an interval $[lo, hi)$, we can binary search for the first value that will have $f$ return $\text{false}$ and then subtract $1$ to get $\text{last_true}$.

This can be easily modified to $text{first_true}$ by inverting the return value of $f$.
