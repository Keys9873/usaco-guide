...
#### Implementation 3 (C++20)
In C++20 and above we can use the ranges and views libraries to perform binary search. Unfortunately, USACO doesn't offer C++20 but it is still available on other online judges like Codeforces.

For a half open interval `\text{lo}, \text{hi}`, we can binary search for the first value that will have `f` return `false` and then subtract 1 to get `last_true`.

This can be easily modified to `first_true` by inverting the return value of `f`.
...
int last_true(int lo, int hi, function<bool(int)> f) {
	return *ranges::partition_point(views::iota(lo, hi), f) - 1;
}
```
</CPPSection>
</LanguageSection>
